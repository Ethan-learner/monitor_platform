  ...
  return (
    <div style={{ padding: 16 }}>
      ...
      <Table<HistoryAlert>
        ...
        columns={[
          { title: '告警名称', dataIndex: 'alertName', width: 90, ellipsis: true, align: 'center' },
          { title: '级别', dataIndex: 'severity', width: 60, align: 'center', render: (s: string) => <Tag color={sevColor[s] || 'default'}>{s}</Tag> },
          { title: '实例', dataIndex: 'instance', width: 100, ellipsis: true, align: 'center' },
          { title: '开始时间', dataIndex: 'startsAt', width: 100, align: 'center', render: (s: string) => s ? new Date(s).toLocaleString() : '-' },
          { title: '结束时间', dataIndex: 'endsAt', width: 100, align: 'center', render: (s: string) => s ? new Date(s).toLocaleString() : '-' },
          { title: '状态', dataIndex: 'status', width: 60, align: 'center', render: (s: string) => <Tag color={s === 'firing' ? 'red' : 'green'}>{s || '-'}</Tag> },
          ...
          { title: '描述', dataIndex: 'summary', ellipsis: true, width: 200, align: 'center' },
        ]}
      />
    </div>
  )
