...
| 序号 | 项目 | 描述 | 状态 |
|------|------|------|------|
| 1 | MySQL 元数据库 | 建库建表完毕，策略/规则/接收人/审计等模块已集成 | 已完成 |
| 2 | 飞书通讯录集成 | 策略配置选人时调用飞书 API 按工号/姓名检索，自动回填邮箱 | 待实施 |
| 3 | 审计日志 | 所有 CRUD 操作写入 audit_log 表，可查询追溯 | 待实施 |
| 4 | 用户 & RBAC | 对接用户表 users，替换 dev mock 登录 | 待实施 |
| 5 | OIDC 单点登录 | 对接公司统一身份平台 | 待实施 |
| 6 | 规则同步服务 | MySQL alert_rules → 服务器 YAML 文件同步 | 待实施 |
| 7 | Webhook 推送记录自动写入 | Webhook 服务写入 webhook_push_log 和 alert_records | 待实施 |
