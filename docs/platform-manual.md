...
| 步骤 | 内容 | 关键产出 |
|------|------|---------|
| 1.1 | 项目骨架搭建 | React+FastAPI 前后端分离架构 |
| 1.2 | 认证体系 | 域控 EIP + 手动账号 + JWT Cookie |
| 1.3 | 用户/角色/权限 | 三角色(ops/dev/mgmt) + 菜单权限过滤 + 级联权限 |
| 1.4 | MySQL 元数据库 | 17 张表全部建成 |
| 1.5 | 登录页 | 深色驾驶舱风格 + 动态背景 + WebSocket 监控元素 |
| 1.6 | 总览首页 | 告警摘要 + 组件健康监控 |
| 1.7 | 用户管理 | 用户 CRUD + 角色分配 + 权限管理 + 登录日志 |

### 第二阶段：告警中心（已完成 ✅）

| 步骤 | 内容 | 关键产出 |
|------|------|---------|
| 2.1 | 活跃告警 | Prometheus API 实时拉取 + 分类展示 |
| 2.2 | 告警规则 CRUD | MySQL 元数据 + SSH YAML 文件 + 三节点热加载 |
| 2.3 | 静默规则管理 | Alertmanager 双写 + 多条件匹配 |
| 2.4 | 历史告警 | MySQL 分页查询 + 多维度筛选 |
| 2.5 | 通知策略 | 分层分级策略 + 结构化多通道多接收人表单 |
| 2.6 | Webhook 集成 | 三节点拓扑 + 推送记录 + 通道未触发区分 |

### 第三阶段：监控面板集成（已完成 ✅）

| 步骤 | 内容 | 关键产出 |
|------|------|---------|
| 3.1 | 主机监控 | Linux/Windows Grafana 看板 iframe |
| 3.2 | 数据库监控 | PMM Overview/Summary/Query Analytics 嵌入 |
| 3.3 | 应用性能 | IOC 应用 + Glowroot JVM/Transactions/Errors |
| 3.4 | 接口监控 | HTTP Blackbox |
| 3.5 | 作业/云服务/日志 | DGC + 华为云 + Loki |
| 3.6 | 指标采集 > 抓取配置 | 完整 CRUD + Dashboard + 采集效果监控（系统配置/数据治理部已录入） |
| 3.7 | 指标采集 > 抓取目标 | Prometheus Targets 实时健康 + 部门过滤 |
| 3.8 | 时序查询 | VictoriaMetrics VMUI 嵌入 |
| 3.9 | 告警推送 | Webhook 服务监控 |

### 第四阶段：抓取配置模块（已完成 ✅）

| 步骤 | 内容 | 关键产出 |
|------|------|---------|
| 4.1 | 数据库设计 | scrape_directories + scrape_targets + scrape_target_health 三张表 |
| 4.2 | 后端 API | 12 个端点（CRUD + 同步 + 健康检测） |
| 4.3 | 文件同步 | SSH SFTP 写入 / 移动 / 删除 YAML 文件 |
| 4.4 | 前端 UI | 左栏文件夹树 + 右栏 Dashboard + 选择模式 |
| 4.5 | 健康监控 | Prometheus 对比 + 多维度匹配 + 30s 缓存 |
| 4.6 | 系统配置迁移 | 13 个系统组件从 static_configs 迁移到 file_sd_configs |
| 4.7 | 禁用/删除流转 | _disabled/_deleted 目录自动管理 |

### 第五阶段：待实施

| 步骤 | 优先级 | 内容 |
|------|--------|------|
| 5.1 | 高 | 飞书通讯录集成（策略选人时按工号/姓名检索） |
| 5.2 | 高 | 告警风暴防护（表达式校验 + 预览匹配确认） |
| 5.3 | 中 | 审计日志查询页（audit_log 已写入，前端页面待开发） |
| 5.4 | 中 | PMM 认证代理（后端自动登录转发，免用户手动登录） |
| 5.5 | 低 | OIDC 单点登录（域控已替代） |

### 第六阶段：全模块测试

| 步骤 | 模块 | 测试内容 | 验证方式 |
|------|------|---------|---------|
| 6.1 | 登录/认证 | 域控登录、手动账号、dev_mock 兜底、JWT 过期、角色权限 | 浏览器操作 |
| 6.2 | 用户管理 | 新增/编辑/删除用户、角色分配、权限映射、登录日志 | 浏览器操作 |
| 6.3 | 总览首页 | 告警摘要统计、组件健康状态、数据刷新 | 浏览器操作 |
| 6.4 | 告警中心 | Alerts 实时拉取、Rules CRUD + SSH 同步、Silences 双写、历史告警分页 | 浏览器 + 服务器验证 |
| 6.5 | 通知策略 | 结构化多通道表单、策略关联规则、severity 同步、禁用/删除级联 | 浏览器 + DB 验证 |
| 6.6 | Webhook | 三节点健康、拓扑图、推送记录展示、通道未触发区分 | 浏览器操作 |
| 6.7 | 抓取配置 | 文件夹 CRUD、配置文件 CRUD、目标 CRUD、选择模式删除、禁用/删除文件流转 | 浏览器 + 服务器文件验证 |
| 6.8 | 抓取 Dashboard | 统计卡片、环形图+图例、各文件目标状态、采集效果监控、未生效列表 | 浏览器操作 |
| 6.9 | 系统配置迁移 | 13 个组件录入验证、prometheus.yml file_sd 切换、Prometheus 采集状态 | API + 服务器验证 |
| 6.10 | 采集效果 | Prometheus 对比匹配、有效/无效/未生效判定、禁用后过滤、删除后清除 | 浏览器 + DB 验证 |
| 6.11 | 权限体系 | ops 全部菜单可见、dev/mgmt 菜单过滤、新增模块默认可见、级联权限 | 浏览器操作 |
| 6.12 | 菜单/导航 | 各菜单入口可达、iframe 嵌入正常、外链打开正确、菜单折叠/展开 | 浏览器操作 |
| 6.13 | 主机/DB/APM/云 | 各 Grafana 看板 iframe 嵌入、PMM 面板、Glowroot 页面 | 浏览器操作 |
| 6.14 | 系统监控 | VictoriaMetrics、Webhook 页面、时序查询 | 浏览器操作 |
| 6.15 | 容错测试 | 后端停止/重启、MySQL 不可达、SSH 不可达、Prometheus 不可达 | 终端 + 浏览器 |
| 6.16 | 性能测试 | Dashboard 数据量、目标列表分页、health 缓存、文件列表加载 | 浏览器 Profile |

### 部署架构总览

```
┌─────────────────────────────────────────────────────────────────┐
│  用户浏览器 (localhost:1009)                                     │
│  ┌───────┐ ┌──────┐ ┌──────────┐ ┌───────┐ ┌──────────────┐   │
│  │ 登录   │ │ 告警  │ │ 系统监控  │ │ 抓取  │ │ 主机/DB/APM  │   │
│  │       │ │ 中心  │ │           │ │ 配置  │ │ /云/日志等   │   │
│  └───────┘ └──────┘ └──────────┘ └───────┘ └──────────────┘   │
└──────────────────────────┬──────────────────────────────────────┘
                           │ /api/* (Vite Proxy)
...
└──────┬──────────────────────┬───────────────┬───────────────────┘
       ...
       ▼                      ▼               ▼
┌──────────────┐  ┌──────────────────┐  ┌──────────────────┐
│ MySQL 8.0    │  │ Prometheus 服务器  │  │ EIP 域控 API     │
│ 元数据库      │  │ 文件同步(读写YAML) │  │ 认证接口          │
│ 17 张业务表   │  │ 三节点 Reload     │  │                  │
└──────────────┘  └──────────────────┘  └──────────────────┘
```
