...
async def list_push_log(limit: int = Query(20, ge=1, le=100)) -> list:
    ...
    try:
        with get_db(readonly=True) as conn:
            cur = conn.cursor()
            cur.execute(
                "SELECT alert_name, instance, channel, status, summary, recipient, created_at FROM webhook_push_log ORDER BY created_at DESC LIMIT %s",
                (limit,),
            )
            rows = cur.fetchall()
            cur.close()
            return [
                {"alertName": r[0], "instance": r[1], "channel": r[2], "status": r[3], "summary": r[4], "recipient": r[5], "createdAt": str(r[6])}
                for r in rows
            ]
